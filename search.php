...
<body>
    ...
    <div class="container my-3" id="cont">
        ...
        <?php 
            $query = $_GET["search"];
            $sql = "SELECT * FROM `product` WHERE MATCH(pName, pDesc) against('$query')"; 
            $result = mysqli_query($conn, $sql);
            while($row = mysqli_fetch_assoc($result)){
                ...
                $pPrice = $row['pPrice'];
                $pDesc = $row['pDesc'];
                $pCategorieId = $row['pCategorieId'];
                
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="card" style="width: 18rem;">
                        <img src="img/pizza-'.$pId. '.jpg" class="card-img-top" alt="image for this product" width="249px" height="270px">
                        <div class="card-body">
                            <h5 class="card-title">' . substr($pName, 0, 20). '...</h5>
                            <h6 style="color: #ff0000">Rs. '.$pPrice. '/-</h6>
                            <p class="card-text">' . substr($pDesc, 0, 29). '...</p>
                            <div class="row justify-content-center">';
                                if($loggedin){
                                    $quaSql = "SELECT `itemQuantity` FROM `viewcart` WHERE pId = '$pId' AND `userId`='$userId'";
                                ...
                                echo '</form>
                                <a href="viewProduct.php?pid=' . $pId . '"><button class="btn btn-primary">Quick View</button></a>
                            </div>
                        </div>
                    </div>
                </div>';

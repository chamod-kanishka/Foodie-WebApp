                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($myrow = mysqli_fetch_assoc($myresult)){
                                        ...
                                        $itemsql = "SELECT * FROM `product` WHERE pId = $pId";
                                        $itemresult = mysqli_query($conn, $itemsql);
                                        $itemrow = mysqli_fetch_assoc($itemresult);
                                        $pName = $itemrow['pName'];
                                        $pPrice = $itemrow['pPrice'];
                                        $pDesc = $itemrow['pDesc'];
                                        $pCategorieId = $itemrow['pCategorieId'];

                                        echo '<tr>
                                                <th scope="row">
                                                    <div class="p-2">
                                                    <img src="img/pizza-'.$pId. '.jpg" alt="" width="70" class="img-fluid rounded shadow-sm">
                                                    <div class="ml-3 d-inline-block align-middle">
                                                        <h5 class="mb-0"> <a href="#" class="text-dark d-inline-block align-middle">'.$pName. '</a></h5><span class="text-muted font-weight-normal font-italic d-block">Rs. ' .$pPrice. '/-</span>
                                                    </div>
                                                    </div>
                                                </th>

...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['addToCart'])) {
        ...
        $existSql = "SELECT * FROM `viewcart` WHERE pId = '$itemId' AND `userId`='$userId'";
        ...
        if($numExistRows > 0){
            ...
            $sql = "INSERT INTO `viewcart` (`pId`, `itemQuantity`, `userId`, `addedDate`) VALUES ('$itemId', '1', '$userId', current_timestamp())";   
    ...
    if(isset($_POST['removeItem'])) {
        $itemId = $_POST["itemId"];
        $sql = "DELETE FROM `viewcart` WHERE `pId`='$itemId' AND `userId`='$userId'";   
        $result = mysqli_query($conn, $sql);
        ...
        if (password_verify($password, $passRow['password'])){ 
            ...
            if ($result){
                ...
                while($addrow = mysqli_fetch_assoc($addResult)){
                    ...
                    $itemSql = "INSERT INTO `orderitems` (`orderId`, `pId`, `itemQuantity`) VALUES ('$orderId', '$pId', '$itemQuantity')";
    ...
    if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
    {
        $pId = $_POST['pId'];
        $qty = $_POST['quantity'];
        $updatesql = "UPDATE `viewcart` SET `itemQuantity`='$qty' WHERE `pId`='$pId' AND `userId`='$userId'";
